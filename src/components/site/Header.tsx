  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-white/85 backdrop-blur-xl border-b border-hairline shadow-[0_2px_20px_-8px_rgba(0,0,0,0.08)]"
          : "bg-transparent"
      }`}
    >
      <div className="mx-auto grid max-w-7xl grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-4 lg:px-8">
        ...
        <nav className="hidden lg:flex items-center justify-center gap-7 text-sm font-medium text-charcoal/85">
          {nav.map((item) => (
            <a
              key={item.label}
              href={item.href}
          ...
          <Link to="/contact" className="relative transition-colors hover:text-brand">Contact</Link>
        </nav>

        <div className="flex items-center gap-2 justify-self-end">
          <a
            href="tel:+919788121526"
            className="hidden sm:inline-flex items-center gap-2 rounded-full bg-brand px-4 py-2.5 text-sm font-semibold text-white shadow-[0_10px_30px_-12px_rgba(178,34,34,0.6)] transition-all hover:bg-brand-hover hover:shadow-[0_14px_34px_-10px_rgba(139,0,0,0.6)]"
          >
